(async function(){
  ...
  const STORAGE_KEY = (window.AMANAYA_STORAGE_KEY || "amanaya:flow:de") + ":v2a";
  ...
  function render(){
    ...
    if (step.type === "text")   html += `<textarea name="field" rows="5" placeholder="">${val?String(val):""}</textarea>`;
  ...
  function nextStep(){
    ...
    if (visIndex < visibleSteps.length - 1) {
      visIndex++; saveLocal(); render();
    } else {
      saveLocal();
      elStep.innerHTML = "<h2>Danke!</h2><p>Du hast alle Fragen beantwortet.</p>";
